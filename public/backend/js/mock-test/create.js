...
$('#m_form').validate({
    ...
    errorPlacement: function (error, element) {
        ...
        else if(element.attr("name").includes("subject_id")){
            error.insertAfter(element.parent().parent().find('.subjectError'));
        }else if(element.attr("name").includes("description")){
            error.insertAfter(element.parent().parent().find('.paperDescriptionError'));
        }else if(element.attr("name").includes("close_instruction")){
            error.insertAfter(element.parent().parent().find('.paperCompleteDescriptionError'));
        }else{
...
$('#m_form').on("submit", function (r) {
        var paperName = $(document).find('.paperDetail').find('input[name^="paper"]');
        ...
        paperName.filter('input[name$="[no_of_section]"]').each(function() {
            $(this).rules("add", {
                required: true,
                maxlength: rule.name_length,
                digits:true,
        ...
        var paperDescription = $(document).find('.paperDetail').find('textarea[name^="paper"]');
        ...
        var paperSubject = $(document).find('.paperDetail').find('select[name^="paper"]');
        ...
        var paperQuestions = $(document).find('.paperDetail').find('input[name^="paper"]');
        ...
        var paperReportQuestion = $(document).find('.paperDetail').find('input[name^="paper"]');
...
$('.paperDetail').on('click','.addSections',function(){
    ...
    var inputName = $(this).attr('data-input_name');
    var no_of_section = $('.paperDetail').find('input[name="'+inputName+'"]').val();
    // if(subject_id != ''){
        $.ajax({
            url:getPaperLayout,
            method:'POST',
