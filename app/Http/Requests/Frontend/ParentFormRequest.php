...
class ParentFormRequest extends FormRequest
{
    ...
    public function rules(Request $request)
    {
        $validator = [
            ...
            'password' =>'required|max:'.config('constant.rules.password_max_length').'|min:'.config('constant.rules.password_min_length'),
            'mobile' =>['required'],
            // 'dob' => 'required',
            // 'address' => 'required',
            // 'city' => 'required',
        ...
        if ($request->has('id') && $request->id != null) {
            ...
            $validator['mobile'][] = 'unique:parents,mobile,' . $request->id.',id,deleted_at,NULL';
        } else {
            $validator['email'][] = 'unique:parents,email,NULL,id,deleted_at,NULL';
            $validator['mobile'][] = 'unique:parents,mobile,NULL,id,deleted_at,NULL';
