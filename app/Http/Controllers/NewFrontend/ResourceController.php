...
class ResourceController extends Controller
{
    ...
    public function index($type = null, $catSlug = null,Request $request)
    {
        try {
            ...
            if($type == 'guidance') {
                ...
                if($request->ajax()) {
                    return view('newfrontend.resources.load_guidance_resources', ['type' => @$type, 'resources' => @$lists]);
                }
                return view('newfrontend.resources.guidance_index', ['type' => @$type, 'category' => @$category, 'resources' => @$lists, 'catSlug' => @$catSlug]);
            } else {
                $lists = $this->resource->whereHas('category', function($q) use($type) {
                            $q->whereSlug($type);
                        })->orderBy('id', 'desc')->paginate(9);
                if($request->ajax()) {
                    return view('newfrontend.resources.load_resources', ['type' => @$type, 'resources' => @$lists]);
                }
                return view('newfrontend.resources.index', ['type' => @$type, 'category' => @$category, 'resources' => @$lists]);
            }
        } catch (Exception $e) {
            abort(404);
        }
    ...
    public function show($slug)
    {
        try {
            ...
            return view('newfrontend.blog.details', ['blog' => @$blog, 'latestBlog' => @$latestResources, 'detailType' => 'guidance', 'category' => @$category]);
        } catch (Exception $e) {
            abort(404);
        }
    }
