...
class BlogController extends Controller
{
    ...
    public function index($catSlug = null, Request $request)
    {
        try {
            ...
            if($request->ajax()) {
                return view('frontend.blog.load_blog', ['blogs' => $lists]);
            }
            // return view
            return view('frontend.blog.index', ['blogs' => @$lists, 'catSlug' => @$catSlug]);
        } catch (Exception $e) {
            // abort(404);
    ...
    public function show($slug)
    {
        try {
            ...
            // retrun view page
            return view('frontend.blog.detail', ['blog' => $blog, 'latestBlog' => $latestBlog, 'detailType' => 'blogs']);
        } catch (Exception $e) {
            return redirect()->back()->with('error',$e->getMessage());
        }
    }
