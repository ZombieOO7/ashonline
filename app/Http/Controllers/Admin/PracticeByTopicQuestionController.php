...
use App\Helpers\PracticeByTopicQuestionHelper;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Imports\PracticeByTopicQuestionImport;
use App\Models\PracticeByTopicAnswer;
...
class PracticeByTopicQuestionController extends BaseController
{
    private $helper;
    public $viewConstant = 'admin.practice-topic-question.';
    public $route = 'practice-topic-question.';

    public function __construct(PracticeByTopicQuestionHelper $helper, PracticeByTopicQuestion $question)
    {
    ...
    public function getBlade(Request $request)
    {
        try{
            ...
            $type = $request->type;
            $question = PracticeByTopicQuestion::where('id', $request->question_id)
                        ->where('type', $type)
    ...
    public function store(Request $request,$uuid=null)
    {
        ...
        try  {
            ...
            foreach ($questionAnswersArr as $questionAnswers) {
                ...
                $quesionList = PracticeByTopicQuestion::updateOrCreate(['uuid'=>@$questionAnswers['uuid']],$request->all());
                ...
                foreach ($questionAnswers['answer'] as $i => $answer) {
                    if(isset($answer['answer']) != null){
                        ...
                        $answer = PracticeByTopicAnswer::updateOrCreate(['question_id' => $quesionList->id, 'id' =>@$answer['answer_id']],$request->all());
                    }
                }
            }
            $this->dbCommit();
            ...
            return redirect()->route('practice-topic-question.index')->with('message', $msg);
        }catch (Exception $e)  {
            $this->dbEnd();
            return redirect()->back()->with('error', $e->getMessage());
            abort('404');
    ...
    public function show(Request $request)
    {
        ...
        $itemQuery = PracticeByTopicQuestion::select('*');
        ...
        // dd($count_total, $count_filter,$questionDataTable);
        return  DataTables::of($questionDataTable)
            ...
            ->addColumn('question_title', function ($questionData) {
            ...
            ->addColumn('progress', function ($questionData) {
    ...
    public function destroy(Request $request)
    {
        $this->dbStart();
        try{
            $question = PracticeByTopicQuestion::where('uuid', $request->id)->delete();
            $this->dbCommit();
    ...
    public function export(Request $request)
    {
        try{
            ...
            $questions = PracticeByTopicQuestion::where(function($query) use($request){
    ...
    public function insert(Request $request)
    {
        ...
        try  {
            ...
            if ($request->hasFile('import_file')) {
                $import = new PracticeByTopicQuestionImport;
                Excel::import($import, $request->import_file);
                $questionIds = $import->questionIds;
            }
            if($request->hasFile('images') && $questionIds != null){
                foreach($request->images as $image){
                    ...
                    foreach($questionIds as $id){
                        ...
                        PracticeByTopicQuestion::where('id',$id)->where('image',strtolower($file[1]))->update(['image'=>@$file[0]]);
                        PracticeByTopicQuestion::where('id',$id)->where('question_image',strtolower($file[1]))->update(['question_image'=>@$file[0]]);
                        PracticeByTopicQuestion::where('id',$id)->where('answer_image',strtolower($file[1]))->update(['answer_image'=>@$file[0]]);
                    }
                }
            }
    ...
    public function detail($uuid){
        try  {
            $question = PracticeByTopicQuestion::where('uuid', $uuid)->firstOrFail();
