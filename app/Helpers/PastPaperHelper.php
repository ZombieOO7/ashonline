...
class PastPaperHelper extends BaseHelper
{
    ...
    public function store(Request $request, $uuid = null)
    {
        ...
        if($request->has('import_file')){
            ...
            $import = new PastPaperQuestionImport($pastPaper->id);
     ...
     * |-----------------------------------------------------
     */
    public function getPaperQuery($subject,$schoolYears)
    {
        // Get Months from september to current month
        $months = $this->getMonths();
        $subject = $this->getSubject($subject);
        $papers = $this->pastPaper::where(['subject_id'=>$subject->id])
