...
@section('content')
@section('title', __('formname.stage.name'))
    ...
    <div class="m-content">
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__body">
                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <select class="form-control" name="action" id='action' aria-invalid="false">
                                ...
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('stage multiple delete')))
                                <option value="{{config('constant.delete')}}">{{__('formname.delete')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('stage multiple active')))
                                <option value="{{config('constant.active')}}">{{__('formname.active')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('stage multiple inactive')))
                            ...
                            <a href="javascript:;" class="btn btn-primary submit_btn" id='action_submit'
                                data-url="{{route('stage_multi_delete')}}"
                                data-table_name="stage_table">{{__('formname.submit')}}</a>
                ...
                <!--begin: Datatable -->
                <table class="table table-striped- table-bordered table-hover table-checkable for_wdth" id="stage_table"
                    data-type="" data-url="{{ route('stage_datatable') }}">
                    <thead>
                        <tr>
                            <th class="nosort">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
                                    placeholder="{{__('formname.stage.title')}}"></th>
                                    <th><input type="text" class="form-control form-control-sm tbl-filter-column"
...
@section('inc_script')
<script>
var url = "{{ route('stage_datatable') }}";
</script>
<script src="{{ asset('backend/js/stages/index.js') }}" type="text/javascript"></script>
@stop