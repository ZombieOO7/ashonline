...
@section('content')
@section('title', __('formname.subscription.label'))
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__body">
                <div class="m-form__content">
                    <h5>{{__('formname.subscription.list')}}</h5>
                </div>
                <hr>
                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            {{-- <select class="form-control" name="action" id='action' aria-invalid="false">
                                ...
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('subscription multiple delete')))
                                <option value="{{config('constant.delete')}}">{{__('formname.delete')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('subscription multiple active')))
                                <option value="{{config('constant.active')}}">{{__('formname.active')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('subscription multiple inactive')))
                            ...
                            {{-- <a href="javascript:;" class="btn btn-primary submit_btn" id='action_submit'
                                data-url="{{route('subscription.multi_delete')}}"
                                data-table_name="subscription_table" data-module_name="Subscription">{{__('formname.submit')}}</a> --}}
                            <button class="btn btn-info" style='margin:0px 0px 0px 12px' id='clr_filter'
                                data-table_name="subscription_table">{{__('formname.clear_filter')}}</button>
                        </div>
                    </div>
                ...
                <!--begin: Datatable -->
                <table class="table table-striped- table-bordered table-hover table-checkable for_wdth" id="subscription_table"
                    data-type="" data-url="{{ route('subscription.datatable') }}">
                    <thead>
                        <tr>
                            <th class="nosort">
                            ...
                            <th>{{__('formname.currency')}}</th>
                            <th>{{__('formname.price')}}</th>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
                                    placeholder="{{__('formname.subscription.title')}}"></th>
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
                                    placeholder="{{__('formname.currency')}}"></th>
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
                                    placeholder="{{__('formname.price')}}"></th>
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
...
@section('inc_script')
<script>
var url = "{{ route('subscription.datatable') }}";
</script>
<script src="{{ asset('backend/js/subscription/index.js') }}" type="text/javascript"></script>
@stop