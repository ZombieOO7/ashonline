...
@section('content')
@section('title', __('formname.topic.label'))
    ...
    <div class="m-content">
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__body">
                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <select class="form-control" name="action" id='action' aria-invalid="false">
                                ...
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('topic multiple delete')))
                                <option value="{{config('constant.delete')}}">{{__('formname.delete')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('topic multiple active')))
                                <option value="{{config('constant.active')}}">{{__('formname.active')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('topic multiple inactive')))
                            ...
                            <a href="javascript:;" class="btn btn-primary submit_btn" id='action_submit'
                                data-url="{{route('topic.multi_delete')}}"
                                data-table_name="topic_table" data-module_name="Topic">{{__('formname.submit')}}</a>
                            <button class="btn btn-info" style='margin:0px 0px 0px 12px' id='clr_filter'
                                data-table_name="topic_table">{{__('formname.clear_filter')}}</button>
                        </div>
                    </div>
                ...
                <!--begin: Datatable -->
                <table class="table table-striped- table-bordered table-hover table-checkable for_wdth" id="topic_table"
                    data-type="" data-url="{{ route('topic.datatable') }}">
                    <thead>
                        <tr>
                            <th class="nosort">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
                                    placeholder="{{__('formname.topic.title')}}"></th>
                            <th><input type="text" class="form-control form-control-sm tbl-filter-column"
...
@section('inc_script')
<script>
var url = "{{ route('topic.datatable') }}";
</script>
<script src="{{ asset('backend/js/topic/index.js') }}" type="text/javascript"></script>
@stop