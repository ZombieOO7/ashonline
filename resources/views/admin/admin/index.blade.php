    ...
    <div class="m-content">
        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__body">
                ...
                <div class="m-portlet__head">
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <select class="form-control" name="action" id='action' aria-invalid="false">
                                ...
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('admin multiple delete')))
                                <option value="{{config('constant.delete')}}">{{trans('formname.delete')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('admin multiple active')))
                                <option value="{{config('constant.active')}}">{{trans('formname.active')}}</option>
                                @endif
                                @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('admin multiple inactive')))
                    ...
                    <div class="m-portlet__head-tools">
                        <ul class="m-portlet__nav">
                            @if((\Auth::guard('admin')->user()->hasRole('superadmin')||\Auth::guard('admin')->user()->can('admin create')))
                ...
                <!--begin: Datatable -->
            <table class="table table-striped- table-bordered table-hover table-checkable" id="admin_table" data-type="" data-url="{{route('admin_datatable')}}">
                    <thead>
                        <tr>
                            <th class="nosort">
